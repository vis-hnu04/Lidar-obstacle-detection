...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr , pcl::PointCloud<pcl::PointXYZ>::Ptr> segmented_plane;
    segmented_plane = point_processor->SegmentPlane(input_cloud, 100,0.2);
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloudClusters = point_processor-> Clustering(segmented_plane.first,1.0,3,30);


    renderPointCloud(viewer,segmented_plane.first,"obstacle",Color(1,0,0));
    renderPointCloud(viewer,segmented_plane.second,"plane",Color(0,1,0));
...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* point_processor, const pcl::PointCloud<pcl::PointXYZI>::Ptr& input_cloud){
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud = point_processor->FilterCloud(input_cloud, 0.13, minPoint, maxPoint);
    ...
    //renderPointCloud(viewer, input_cloud,"input");
